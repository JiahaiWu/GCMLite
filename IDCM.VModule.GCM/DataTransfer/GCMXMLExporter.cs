...
namespace IDCM.DataTransfer
{
    class GCMXMLExporter
    {
        ...
        /// 根据历史查询条件导出目标文本数据集
        /// <returns></returns>
        public bool exportXML(GCMTableCache ctcache, string filepath)
        {
            try
            {
                StringBuilder strbuilder = new StringBuilder();
                int tcount = ctcache.getOverViewRowCount();
                using (FileStream fs = new FileStream(filepath, FileMode.Create))
                {
                    XmlDocument xmlDoc = new XmlDocument();
                    strbuilder.Append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
                    strbuilder.Append("<strains>\r\n");
                    Dictionary<string, int> maps = ctcache.getOverViewIAttrMapping();
                    ///////////////////
                    int ridx = 0;
                    while (ridx < tcount)
                    {
                        Dictionary<int, string> drow = ctcache.getOverViewIRow(ridx);
                        if (drow != null)
                        {
        ...
        /// <returns></returns>
        internal bool exportXML(GCMTableCache ctcache, string filepath, int[] selectedRows)
        {
            try
            {
                StringBuilder strbuilder = new StringBuilder();
                ...
                using (FileStream fs = new FileStream(filepath, FileMode.Create))
                {
                    ...
                    for (int ridx = selectedRows.Length - 1; ridx >= 0; ridx--)
                    {
                        ...
                        Dictionary<int, string> drow = ctcache.getOverViewIRow(lindex);
                        if (drow != null)
                        {
                            XmlElement xmlEle = convertToXML(xmlDoc, maps, drow);
                            strbuilder.Append(xmlEle.OuterXml).Append("\r\n");
        ...
        private static XmlElement convertToXML(XmlDocument xmlDoc, Dictionary<string, int> maps, Dictionary<int, string> row)
        {
            ...
            {
                string value = null;
                if (row.TryGetValue(mapEntry.Value, out value))
                {
        ...
        internal bool exportGCMXML(GCMTableCache ctcache, MemoryStream xmlStream, Dictionary<string, string> dataMapping, int[] selectedRows)
        {
            try
            {
                StringBuilder strbuilder = new StringBuilder();
                int count = 0;
                {
                    ...
                    {
                        int lindex = selectedRows[ridx];
                        Dictionary<string, string> drow = ctcache.getOverViewRow(lindex);
                        if (drow != null)
                        {
                            XmlElement xmlEle = convertToXML(xmlDoc, dataMapping, drow);
                            strbuilder.Append(xmlEle.OuterXml).Append("\r\n");
